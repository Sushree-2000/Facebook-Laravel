            ...
            <div class="mypost_left py-5">
                @foreach ($posts as $post)
                    ...
                    <div class="info">
                        {{ $post->textpost }} <br>
                        {{-- {{ $post->imagepost }} --}}
                        <img src="{{ asset('postedImg/' . $post->imagepost) }}" alt="">
                    </div>
                    <div class="interaction mb-2">
                        <a href=" {{url('like',['id'=> $post->id])}} " class="text-blue-600 hover:underline mr-1">Like({{$post->likes}})</a>|
                        <a href=" {{url('dislike',['id'=> $post->id])}} " class="text-blue-600 hover:underline mr-1">Dislike({{$post->dislikes}})</a>|
